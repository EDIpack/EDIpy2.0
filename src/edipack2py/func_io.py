...
# superconductive phi: argument
def get_arg(self, ilat=None, iorb=None, jorb=None):
    """
    This function returns the phase :math:`\\theta` of the superconductive order \
    parameter :math:`\\phi = \\langle c_{\\uparrow} c_{\\downarrow} \\rangle`
   ...
   :return: :math:`\\theta`. The full tensor has dimensions [ :code:`Nlat`, \
   :data:`Norb`, :data:`Norb`]. Depending on which keyword arguments are \
    (or not) provided, this is sliced on the corresponding axis.
   :rtype: float **or** np.array(dtype=float)
   ...
   """
    ...
    if self.Nineq == 0:
        argvec = np.zeros((aux_norb, aux_norb), dtype=float, order="F")
        ed_get_argsc_n2_wrap(argvec)
        argvec = np.ascontiguousarray(argvec)

        if ilat is not None:
            raise ValueError("ilat cannot be none for single-impurity DMFT")
        elif iorb is not None and jorb is not None:
            return argvec[iorb, jorb]
        elif iorb is not None and jorb is None:
            return argvec[iorb, :]
        elif jorb is not None and iorb is None:
            return argvec[:, jorb]
        ...
        if self.has_ineq:
            ...
            ed_get_docc_n3_wrap(argvec, self.Nineq)
            argvec = np.ascontiguousarray(argvec)

            if ilat is not None:
                if iorb is not None and jorb is not None:
                    return argvec[ilat, iorb, jorb]
                if iorb is not None and jorb is None:
                    return argvec[ilat, iorb, :]
                if jorb is not None and iorb is None:
                    return argvec[ilat, :, jorb]
                else:
                    return argvec[ilat, :, :]
            else:
                if iorb is not None and jorb is not None:
